...
set -euo pipefail
echo "== Network sanity =="; curl -sS https://example.com >/dev/null && echo "OK: Internet reachable." || { echo "❌ No internet from Replit container."; exit 1; }
...
echo "== Health probe =="; for p in /v1/health /health /ping /v1/ping; do code=$(curl -sS -o /tmp/h -w "%{http_code}" "${SFS_API_URL%/}${p}" || true); echo "${SFS_API_URL%/}${p} → HTTP $code"; [ "$code" != "000" ] && break; done
if [ -n "${SFS_TOKEN:-}" ] || [ -n "${SFS_API_KEY:-}" ]; then
  ...
  for p in /v1/me /v1/projects /me; do code=$(curl -sS -o /tmp/a -w "%{http_code}" "${H[@]}" "${SFS_API_URL%/}${p}" || true); echo "Auth ${p} → HTTP $code"; [ "$code" -ge 200 ] && [ "$code" -lt 300 ] && { echo "✅ Auth looks good."; exit 0; }; done
  echo "❌ Auth failed. Check header type (Bearer vs x-api-key) and scopes."; exit 3
else
  echo "ℹ️ Add SFS_TOKEN or SFS_API_KEY in 🔐 Secrets and re-run."; exit 4
fi
